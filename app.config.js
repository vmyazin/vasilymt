...
/**
 * Middleware to set site profile based on domain or environment variable.
 * Domain takes precedence over ACTIVE_PROFILE env var.
 */
const setSiteProfile = (req, res, next) => {
    ...
    // Try domain-based detection first, then fall back to env var
    const host = req.get('host');
    const domainProfile = getProfileFromHost(host);
    const activeProfile = domainProfile || getEnvVar('ACTIVE_PROFILE', 'professional');

    debugLogger.log('profile-selection', `Host: ${host}, Selected profile: ${activeProfile}`);

    if (!siteProfiles[activeProfile]) {
        debugLogger.log('profile-warning', `Profile "${activeProfile}" not found, falling back to professional`);
